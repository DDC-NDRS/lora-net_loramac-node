...
static MlmeConfirmQueue_t* GetElement(Mlme_t request, MlmeConfirmQueue_t* bufferStart, MlmeConfirmQueue_t* bufferEnd) {
    ...
    uint8_t MlmeConfirmQueueCnt = ConfirmQueueCtx.Nvm.MlmeConfirmQueueCnt;

    if (IsListEmpty(MlmeConfirmQueueCnt) == true) {
        return (NULL);
    }

    for (uint8_t elementCnt = 0; elementCnt < MlmeConfirmQueueCnt; elementCnt++) {
...
bool LoRaMacConfirmQueueAdd(MlmeConfirmQueue_t* mlmeConfirm) {
    ...
    MlmeConfirmQueue_t* BufferEnd = ConfirmQueueCtx.BufferEnd;
    ...
    BufferEnd->ReadyToHandle = mlmeConfirm->ReadyToHandle;

    // Increase counter
    ConfirmQueueCtx.Nvm.MlmeConfirmQueueCnt++;

    // Update end pointer
    ConfirmQueueCtx.BufferEnd = IncreaseBufferPointer(BufferEnd);

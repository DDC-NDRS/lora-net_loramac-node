...
static uint8_t CountChannels(uint16_t mask, uint8_t nbBits) {
    uint_fast8_t nbActiveBits = 0;

    for (uint_fast8_t j = 0; j < nbBits; j++) {
        if ((mask & (1 << j)) == (1 << j)) {
            nbActiveBits++;
        }
    }
...
bool RegionCommonChanVerifyDr(uint8_t nbChannels, uint16_t* channelsMask, int8_t dr,
                              int8_t minDr, int8_t maxDr,
                              ChannelParams_t* channels) {
    uint8_t isInRange;

    isInRange = RegionCommonValueInRange(dr, minDr, maxDr);
    if (isInRange == 0) {
        return (false);
    }

    for (uint_fast8_t i = 0, k = 0; i < nbChannels; i += 16, k++) {
        for (uint_fast8_t j = 0; j < 16; j++) {
            if (((channelsMask[k] & (1 << j)) != 0)) {
                // Check datarate validity for enabled channels
                struct sFields* Fields = &(channels[i + j].DrRange.Fields);

                isInRange = RegionCommonValueInRange(dr,
                                                     (Fields->Min & 0x0F),
                                                     (Fields->Max & 0x0F));
                if (isInRange == 1) {
...
uint8_t RegionCommonCountChannels(uint16_t* channelsMask, uint8_t startIdx, uint8_t stopIdx) {
    uint_fast8_t nbChannels = 0;

    if (channelsMask == NULL) {
        return (0);
    }

    for (uint_fast8_t i = startIdx; i < stopIdx; i++) {
...
void RegionCommonSetBandTxDone(Band_t* band, TimerTime_t lastTxAirTime, bool joined,
...
TimerTime_t RegionCommonUpdateBandTimeOff(bool joined, Band_t* bands,
    ...
    for (uint_fast8_t i = 0; i < nbBands; i++) {
...
void RegionCommonComputeRxWindowParameters(uint32_t tSymbolInUs, uint8_t minRxSymbols, uint32_t rxErrorInMs,
                                           uint32_t wakeUpTimeInMs, uint32_t* windowTimeoutInSymbols,
                                           int32_t* windowOffsetInMs) {
    uint32_t Timeout;

    Timeout = MAX(DIV_CEIL(((2 * minRxSymbols - 8) * tSymbolInUs + 2 * (rxErrorInMs * 1000)), tSymbolInUs),
                  minRxSymbols);  // Computed number of symbols

    *windowOffsetInMs = (int32_t)DIV_CEIL((int32_t)(4 * tSymbolInUs) -
                                          (int32_t)DIV_CEIL((Timeout * tSymbolInUs), 2) -
                                          (int32_t)(wakeUpTimeInMs * 1000), 1000);

    *windowTimeoutInSymbols = Timeout;
}
...
void RegionCommonRxBeaconSetup(RegionCommonRxBeaconSetupParams_t* rxBeaconSetupParams) {
    ...
    // Setup radio
    Radio.SetRxConfig(MODEM_LORA, rxBeaconSetupParams->BeaconChannelBW,
                      datarate, 1,
                      0, 10,
                      rxBeaconSetupParams->SymbolTimeout, true,
...
void RegionCommonCountNbOfEnabledChannels(RegionCommonCountNbOfEnabledChannelsParams_t* countNbOfEnabledChannelsParams,
    ...
    for (uint_fast8_t i = 0, k = 0; i < countNbOfEnabledChannelsParams->MaxNbChannels; i += 16, k++) {
        for (uint_fast8_t j = 0; j < 16; j++) {
            if ((countNbOfEnabledChannelsParams->ChannelsMask[k] & (1 << j)) != 0) {
                ...
                if (countNbOfEnabledChannelsParams->Bands[countNbOfEnabledChannelsParams->Channels[i + j].Band]
                        .ReadyForTransmission == false) { // Check if the band is available for transmission
    ...
    if (params->CurrentDr == params->MinDr) {
        ...
        do {
            drLocal = (drLocal - 1);
                 ...
                 (RegionCommonChanVerifyDr(params->NbChannels, params->ChannelsMask, drLocal, params->MinDr,
                                           params->MaxDr, params->Channels) == false));
...
uint32_t RegionCommonGetBandwidth(uint32_t drIndex, uint32_t const* bandwidths) {

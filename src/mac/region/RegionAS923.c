...
static TimerTime_t GetTimeOnAir(int8_t datarate, uint16_t pktLen) {
    ...
    TimerTime_t timeOnAir;

    if (datarate == DR_7) { // High Speed FSK channel
        timeOnAir = Radio.TimeOnAir(MODEM_FSK, bandwidth,
                                    (phyDr * 1000L), 0,
                                    5, false, pktLen,
                                    true);
    }
    else {
        timeOnAir = Radio.TimeOnAir(MODEM_LORA, bandwidth,
                                    phyDr, 1,
                                    8, false, pktLen,
                                    true);
    }
    ...
    switch (phyAttribute) {
        ...
        case PHY_TX_DR : {
            if (verify->DatarateParams.UplinkDwellTime == 0) {
                return RegionCommonValueInRange(verify->DatarateParams.Datarate,
                                                AS923_TX_MIN_DATARATE, AS923_TX_MAX_DATARATE);
            }
            else {
                return RegionCommonValueInRange(verify->DatarateParams.Datarate,
                                                AS923_DWELL_LIMIT_DATARATE, AS923_TX_MAX_DATARATE);
            }
        }
        ...
        case PHY_RX_DR : {
            if (verify->DatarateParams.DownlinkDwellTime == 0) {
                return RegionCommonValueInRange(verify->DatarateParams.Datarate,
                                                AS923_RX_MIN_DATARATE, AS923_RX_MAX_DATARATE);
            }
            else {
                return RegionCommonValueInRange(verify->DatarateParams.Datarate,
                                                AS923_DWELL_LIMIT_DATARATE, AS923_RX_MAX_DATARATE);
            }
        }
        ...
        case PHY_TX_POWER : {
            ...
            return RegionCommonValueInRange(verify->TxPower,
                                            AS923_MAX_TX_POWER, AS923_MIN_TX_POWER);
        }
...
void RegionAS923ApplyCFList(ApplyCFListParams_t* applyCFList) {
    ...
    for (uint_fast8_t i = 0, chanIdx = AS923_NUMB_DEFAULT_CHANNELS;
         chanIdx < AS923_MAX_NB_CHANNELS;
         i += 3, chanIdx++) {
        if (chanIdx < (AS923_NUMB_CHANNELS_CF_LIST + AS923_NUMB_DEFAULT_CHANNELS)) {
            // Channel frequency
            uint32_t Frequency;

            Frequency  =  (uint32_t)applyCFList->Payload[i];
            Frequency |= ((uint32_t)applyCFList->Payload[i + 1] << 8);
            Frequency |= ((uint32_t)applyCFList->Payload[i + 2] << 16);
            Frequency *= 100;
            newChannel.Frequency = Frequency;
...
void RegionAS923ComputeRxWindowParameters(int8_t datarate, uint8_t minRxSymbols, uint32_t rxError,
    ...
    RegionCommonComputeRxWindowParameters(tSymbolInUs, minRxSymbols, rxError, Radio.GetWakeupTime(),
...
bool RegionAS923RxConfig(RxConfigParams_t* rxConfig, int8_t* datarate) {
    ...
    if (dr == DR_7) {
        modem = MODEM_FSK;
        Radio.SetRxConfig(modem, 50000,
                          phyDr * 1000L, 0,
                          83333, 5,
                          rxConfig->WindowTimeout, false,
                          0,
                          true, 0, 0,
        ...
        modem = MODEM_LORA;
        Radio.SetRxConfig(modem, rxConfig->Bandwidth,
                          phyDr, 1,
                          0, 8,
                          rxConfig->WindowTimeout, false,
                          0,
...
bool RegionAS923TxConfig(TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir) {
    ...
    if (txConfig->Datarate == DR_7) { // High Speed FSK channel
        modem = MODEM_FSK;
        Radio.SetTxConfig(modem, phyTxPower, 25000,
                          bandwidth, (phyDr * 1000L),
                          0, 5,
                          false, true, 0,
                          0, false, 4000);
    }
        ...
        modem = MODEM_LORA;
        Radio.SetTxConfig(modem, phyTxPower, 0,
                          bandwidth, phyDr,
                          1, 8,
                          false, true, 0,
                          0, false, 4000);
...
uint8_t RegionAS923LinkAdrReq(LinkAdrReqParams_t* linkAdrReq, int8_t* drOut,
                              int8_t* txPowOut, uint8_t* nbRepOut, uint8_t* nbBytesParsed) {
    uint8_t status = 0x07;
    ...
    while (bytesProcessed < linkAdrReq->PayloadSize) {
        ...
        if ((linkAdrParams.ChMaskCtrl == 0) && (chMask == 0)) {
            ...
            for (uint_fast8_t i = 0; i < AS923_MAX_NB_CHANNELS; i++) {
                if (linkAdrParams.ChMaskCtrl == 6) {
                    ...
                    if (((chMask & (1 << i)) != 0) &&
                        (RegionNvmGroup2->Channels[i].Frequency == 0)) { // Trying to enable an undefined channel
                        status &= 0xFE;                                  // Channel mask KO
                    }
                }
            }
...
uint8_t RegionAS923RxParamSetupReq(RxParamSetupReqParams_t* rxParamSetupReq) {
    ...
    // Verify datarate
    isInRange = RegionCommonValueInRange(rxParamSetupReq->Datarate,
                                         AS923_RX_MIN_DATARATE, AS923_RX_MAX_DATARATE);
    if (isInRange == 0) {
    ...
    isInRange = RegionCommonValueInRange(rxParamSetupReq->DrOffset,
                                         AS923_MIN_RX1_DR_OFFSET, AS923_MAX_RX1_DR_OFFSET);
    if (isInRange == 0) {
...
LoRaMacStatus_t RegionAS923NextChannel(NextChanParams_t* nextChanParams,
    ...
    if (RegionCommonCountChannels(RegionNvmGroup2->ChannelsMask, 0, 1) == 0) { // Reactivate default channels
    ...
    if (status == LORAMAC_STATUS_OK) {
        ...
        for (uint_fast8_t i = 0, j = randr(0, nbEnabledChannels - 1); i < AS923_MAX_NB_CHANNELS; i++) {
...
LoRaMacStatus_t RegionAS923ChannelAdd(ChannelAddParams_t* channelAdd) {
    ...
    if (RegionCommonValueInRange(channelAdd->NewChannel->DrRange.Fields.Min,
                                 AS923_TX_MIN_DATARATE, AS923_TX_MAX_DATARATE) == false) {
        drInvalid = true;
    }
    ...
    memcpy1((uint8_t*)&(RegionNvmGroup2->Channels[id]), (uint8_t*)channelAdd->NewChannel,
...
void RegionAS923RxBeaconSetup(RxBeaconSetup_t* rxBeaconSetup, uint8_t* outDr) {
    RegionCommonRxBeaconSetupParams_t Setup;

    Setup.Datarates       = DataratesAS923;
    Setup.Frequency       = rxBeaconSetup->Frequency;
    Setup.BeaconSize      = AS923_BEACON_SIZE;
    Setup.BeaconDatarate  = AS923_BEACON_CHANNEL_DR;
    Setup.BeaconChannelBW = AS923_BEACON_CHANNEL_BW;
    Setup.RxTime          = rxBeaconSetup->RxTime;
    Setup.SymbolTimeout   = rxBeaconSetup->SymbolTimeout;

    RegionCommonRxBeaconSetup(&Setup);

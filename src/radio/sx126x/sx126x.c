...
void SX126xSetModulationParams(ModulationParams_t* modulationParams) {
    ...
    switch (modulationParams->PacketType) {
        ...
        case PACKET_TYPE_LORA : {
            n = 4;
            struct ModulationParamsLoRa_s* params = &modulationParams->Params.LoRa;
            buf[0] = params->SpreadingFactor;
            buf[1] = params->Bandwidth;
            buf[2] = params->CodingRate;
            buf[3] = params->LowDatarateOptimize;
...
void SX126xSetPacketParams(PacketParams_t* packetParams) {
    ...
    switch (packetParams->PacketType) {
        case PACKET_TYPE_GFSK : {
            struct PacketParamsGfsk_s* ParamsGfsk = &packetParams->Params.Gfsk;

            if (ParamsGfsk->CrcLength == RADIO_CRC_2_BYTES_IBM) {
                SX126xSetCrcSeed(CRC_IBM_SEED);
                SX126xSetCrcPolynomial(CRC_POLYNOMIAL_IBM);
                crcVal = RADIO_CRC_2_BYTES;
            }
            else if (ParamsGfsk->CrcLength == RADIO_CRC_2_BYTES_CCIT) {
                SX126xSetCrcSeed(CRC_CCITT_SEED);
                SX126xSetCrcPolynomial(CRC_POLYNOMIAL_CCITT);
                crcVal = RADIO_CRC_2_BYTES_INV;
            }
            else {
                crcVal = ParamsGfsk->CrcLength;
            }

            n = 9;
            buf[0] = (ParamsGfsk->PreambleLength >> 8) & 0xFF;
            buf[1] = ParamsGfsk->PreambleLength;
            buf[2] = ParamsGfsk->PreambleMinDetect;
            buf[3] = (ParamsGfsk->SyncWordLength /*<< 3*/); // convert from byte to bit
            buf[4] = ParamsGfsk->AddrComp;
            buf[5] = ParamsGfsk->HeaderType;
            buf[6] = ParamsGfsk->PayloadLength;
            buf[7] = crcVal;
            buf[8] = ParamsGfsk->DcFree;
            break;
        }

        case PACKET_TYPE_LORA : {
            n = 6;
            struct PacketParamsLoRa_s* ParamsLoRa = &packetParams->Params.LoRa;

            buf[0] = (ParamsLoRa->PreambleLength >> 8) & 0xFF;
            buf[1] = ParamsLoRa->PreambleLength;
            buf[2] = LoRaHeaderType = ParamsLoRa->HeaderType;
            buf[3] = ParamsLoRa->PayloadLength;
            buf[4] = ParamsLoRa->CrcMode;
            buf[5] = ParamsLoRa->InvertIQ;
            break;
        }
...
RadioError_t SX126xGetDeviceErrors(void) {
    ...
    error.Fields.PaRamp     =  err[0] & 1;
    error.Fields.PllLock    = (err[1] >> 6) & 1;
    error.Fields.XoscStart  = (err[1] >> 5) & 1;
    error.Fields.ImgCalib   = (err[1] >> 4) & 1;
    error.Fields.AdcCalib   = (err[1] >> 3) & 1;
    error.Fields.PllCalib   = (err[1] >> 2) & 1;
    error.Fields.Rc13mCalib = (err[1] >> 1) & 1;
    error.Fields.Rc64kCalib =  err[1] & 1;

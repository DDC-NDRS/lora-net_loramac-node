...
__noinit uint8_t RadioRxPayload[255];
...
bool RadioIsChannelFree(uint32_t freq, uint32_t rxBandwidth, int16_t rssiThresh, uint32_t maxCarrierSenseTime) {
    ...
    RadioSetRxConfig(MODEM_FSK, rxBandwidth,
                     600, 0,
                     rxBandwidth, 3,
                     0, false,
                     0,
...
void RadioSetRxConfig(RadioModems_t modem, uint32_t bandwidth,
                      ...
                      bool crcOn, bool freqHopOn, uint8_t hopPeriod,
                      bool iqInverted, bool rxContinuous) {
    ModulationParams_t* Modulation = &SX126x.ModulationParams;
    PacketParams_t* Packet = &SX126x.PacketParams;
    ...
    switch (modem) {
        case MODEM_FSK : {
            ...
            PacketGfsk->PayloadLength = MaxPayloadLength;
            if (crcOn == true) {
        ...
        case MODEM_LORA : {
            ...
            if (PacketLoRa->InvertIQ == LORA_IQ_INVERTED) {
